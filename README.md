...
$$\boldsymbol{\tau}_{\mathrm{gyro}} = \boldsymbol{\omega} \times (\mathbf{I}\boldsymbol{\omega})$$

* **Rotational Acceleration:** Angular acceleration is evaluated in local space as:

$$\boldsymbol{\alpha} = \mathbf{I}^{-1}(\boldsymbol{\tau}_{\mathrm{local}} - \boldsymbol{\tau}_{\mathrm{gyro}})$$
...
$$\Delta\mathbf{r}_{\mathrm{CoM}} = \frac{-m_{\mathrm{part}} \mathbf{r}_{\mathrm{part}}}{M_{\mathrm{new}}}$$
...
$$\mathbf{I}_{\mathrm{shift}} = m \left( (\mathbf{r} \cdot \mathbf{r})\mathbf{E} - \mathbf{r} \otimes \mathbf{r} \right)$$
...
\mathbf{K}_{p,\mathrm{rate}}
(\boldsymbol{\omega}_{\mathrm{target}} - \boldsymbol{\omega}) - \mathbf{K}_{d,\mathrm{rate}} \boldsymbol{\omega}
$$
...
$$
\begin{bmatrix} T_{\mathrm{total}} \\ \tau_{x} \\ \tau_{z} \end{bmatrix} =
\begin{bmatrix} 1 & 1 & 1 \\ -z_{1} & -z_{2} & -z_{3} \\ x_{1} & x_{2} & x_{3} \end{bmatrix}
...
$$m_{\mathrm{eff}} = \frac{m_{\mathrm{drone}} \cdot m_{\mathrm{load}}}{m_{\mathrm{drone}} + m_{\mathrm{load}}}$$

* **Critical Damping Coefficient:** Computed continuously to adapt to attached loads:

$$c_{\mathrm{crit}} = 2 \sqrt{k \cdot m_{\mathrm{eff}}}, \quad c = c_{\mathrm{crit}} \cdot \zeta$$
...
$$T_{\mathrm{effective}} = T \cdot (1 + 0.5 e^{-2h})$$
...
$$v_{\mathrm{induced}} = \sqrt{\frac{T}{2 \rho A}}$$
...
$$\boldsymbol{\tau}_{\mathrm{cable}} = \mathbf{r}_{\mathrm{hook}} \times \mathbf{F}_{\mathrm{cable}}$$
